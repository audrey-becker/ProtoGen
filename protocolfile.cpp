...
void ProtocolFile::writePyIncludeDirective(const std::string& include, const std::string& comment)
{
    ...
    // We try to group all the import statements together
    std::size_t index = contents.rfind("import *");
    if(index < contents.size())
    {
        // Find the end of the line
        index = contents.find("\n", index);
